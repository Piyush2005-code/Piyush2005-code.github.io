...
// ── SYSTEMS-HACKER PORTFOLIO ADDITIONS ──
    ...
    const TELEMETRY_LOGS = [
      // 0: ARM64 Unikernel
      [
        "[boot] Initializing ARMv8-A EL3 to EL1 boot sequence...",
        "[mmu] Set up page tables (L0, L1, L2). Translation enabled.",
        "[irq] Configuring Generic Interrupt Controller (GICv2)...",
        "[mmu] Identity map memory regions, pre-allocated 128MB arena.",
        "[sched] Context size: 104B. Cooperative scheduler started.",
        "[onnx] Parsing ONNX protobuf file structure...",
        "[onnx] Model loaded successfully: 42 operations verified.",
        "[simd] Cache alignment checked. NEON registers initialized.",
        "[prof] Starting Conv2D / GEMM matrix multiply bench...",
        "[prof] Inference complete: 269.45 microseconds.",
        "[sys] Unikernel status: deterministic execution loop [OK]."
      ],
      // 1: Counsel.ai
      [
        "[model] Deferring vLLM loading to local server: port 8000",
        "[distilroberta] Model loaded into CUDA:0 successfully.",
        "[ncdm] PyTorch trait estimation model initialized (11 dimensions).",
        "[semantic] SentenceTransformers connecting to Pinecone vector DB...",
        "[semantic] Query vector generated. Shape: [1, 384].",
        "[semantic] Retrieval query matched 5 colleges in 18ms.",
        "[postgres] Filtering by cutoff and fees constraints... 3 matches.",
        "[agentic] Mistral multi-turn counselling logic: state synchronized.",
        "[mcp] FastMCP server active: exposed 4 database diagnostic tools."
      ],
      // 2: OS Scheduling Simulator
      [
        "[simulator] React component tree mounted.",
        "[sim] Spawning 5 processes with random burst and priority values.",
        "[algo] Running First-Come First-Served scheduling pass...",
        "[algo] Running Round Robin pass (Time Quantum = 4ms)...",
        "[algo] Running Non-Preemptive Priority pass...",
        "[prof] Average Turnaround Time (RR): 14.8ms",
        "[prof] Average Waiting Time (RR): 6.2ms",
        "[gantt] Drawing SVG chart lines for FCFS sequence...",
        "[ipc] Sending scheduling arrays to Electron main process."
      ],
      // 3: JARVIS Voice Assistant
      [
        "[audio] AudioFlamingo3 speech processing pipeline live.",
        "[cuda] CUDA context created on GPU:0 (NVIDIA A5000).",
        "[cuda] CUDA kernels optimized. Cache warming completed.",
        "[whisper] Real-time audio stream decoder hooked.",
        "[rag] Initializing query search on local vector database...",
        "[vllm] Pipeline execution triggered with sub-second constraint.",
        "[whisper] Decoding completed in 42ms.",
        "[vllm] LLM generation response received: 84 tokens.",
        "[sys] Response stream sent to TTS module. Latency: 0.85s."
      ],
      // 4: LLM Chart Gen
      [
        "[server] Node.js backend listening on port 5000",
        "[pdf] Uploaded document received. File size: 2.4MB",
        "[pdf] PDF parsing engine extracting text buffers...",
        "[trans] Deployed semantic text segmenter...",
        "[llm] Formatting JSON payload for local LLM completion...",
        "[llm] Prompt tokens: 4096, Output tokens: 1024",
        "[classifier] ML-based data type classification: NUMERIC",
        "[chart] Matching layout schema: Bar chart selected.",
        "[chart] Rendering SVG output: Success [OK]."
      ],
      // 5: Crop Stress
      [
        "[torch] PyTorch runtime loaded. CUDA/MPS acceleration active.",
        "[unet] Initializing U-Net semantic segmentation network (7.7M params).",
        "[synthetic] Loading augmentations: Gaussian blend + Rotations.",
        "[train] Epoch 100/100 completed. BCE Loss: 0.042, Dice: 0.945",
        "[cv] OpenCV capture opened for aerial_footage_08.mp4.",
        "[infer] Batch size: 8, FPS: 45.3",
        "[cv] Writing video stream mask overlay overlay_output.mp4."
      ],
      // 6: Wing Design
      [
        "[cad] Parsing wing profile SolidWorks XML coordinates...",
        "[mesh] Generating multi-block structured mesh inside domain...",
        "[mesh] Mesh verification: 1.2M nodes, max skewness: 0.22",
        "[solver] Loading ANSYS Fluent solver routines...",
        "[solver] Target parameters: AoA = 8°, Re = 1.5e5",
        "[solver] Iteration 500/500 converged. Residuals < 1e-6",
        "[prof] Max Lift Coefficient (CL) calculated: 8.1258",
        "[prof] Lift/Drag ratio (L/D): 18.42"
      ],
      // 7: Quadcopter CAD
      [
        "[cad] Loading quadcopter model database from Fusion 360 API...",
        "[fea] Setting material density: Carbon Fiber (1.6g/cm³)",
        "[fea] Stress analysis: loading loads on motor arms (4.5G crash)",
        "[fea] Max deflection: 0.12mm (Safety factor: 3.4)",
        "[geom] Center of Gravity (CoG) coordinate: [0.0, 0.0, 1.45]",
        "[autopilot] Mapping hardware sensor mounts (IMU, Baro, GPS)",
        "[sys] Rigidity validation complete: Weight target 420g met."
      ]
    ];

    function startTelemetryStream(idx) {
      if (telemetryIntervalId) {
        clearInterval(telemetryIntervalId);
      }
      
      const consoleEl = document.getElementById('pm-telemetry-console');
      if (!consoleEl) return;
      consoleEl.textContent = '';
      
      const logs = TELEMETRY_LOGS[idx] || ["[sys] No telemetry log available for this project index."];
      let logIdx = 0;
      
      function getLogClass(prefix) {
        if (prefix === '[sys]' || prefix === '[boot]') return 'telemetry-log-green';
        if (prefix === '[err]' || prefix === '[warn]') return 'telemetry-log-yellow';
        if (['[mmu]', '[irq]', '[sched]', '[cuda]', '[mesh]', '[fea]'].includes(prefix)) return 'telemetry-log-blue';
        return '';
      }
      ...
      function appendLog() {
        if (logIdx < logs.length) {
          ...
          if (match) {
            ...
            const rest = match[2];
            const cls = getLogClass(prefix);
            div.innerHTML = `<span class="${cls}">${prefix}</span> ${rest}`;
          } else {
      ...
      telemetryIntervalId = setInterval(appendLog, 200);
    }
    ...
    function openModal(idx) {
      ...
      // Start real-time project telemetry console stream
      startTelemetryStream(idx);
    }
    function closeModal() {
      if (telemetryIntervalId) {
        clearInterval(telemetryIntervalId);
...
Available: hero, about, research, projects, skills, contact`, "term-out-error");
        ...
        if (cmd === 'resume') {
          ...
          window.open("https://doc-10-74-apps-viewer.googleusercontent.com/viewer/secure/pdf/64fltson0mi7n692h48inmd0lvea6od4/3h8d4j7k7r0md2iaggdq6vfoudktue2i/1779780525000/drive/07795496189542291328/ACFrOgCCiFsu_r-0ehHy7r_FypQwcuSpALJQjERZmlOPFApmWv5GASzdC-NXu1GT3NlFxD0UHCWrrxfsiTGPJhSpxLzWbUodw3Oq26YYG87hQRf9NjPwLfSvFhntBNA3doFJb3E_Oxzx6f27S2zU4vw40fy5sfPtEztskXVkcNdMS2Mlj9oJ_l3RTyc1Ck5rZ0-DI4GzNYLigNr62gl7eZ418MxFTFjg_rd-ir3d-ijxQ6eQuBYj6dZu9H1KTu4=?print=true&authuser=0&nonce=llngrr2qfckku&user=07795496189542291328&hash=r99d6i0lo1jogh1uib7abisvlal6en68", "_blank");
          return;
        }
